...
from services.sql import df_submission

from services.utilities import calculate_center, calculate_zoom, get_page_note
...
default_year = get_latest_year_with_data(df_submission, year_column="svyYear")


# ✅ **Define Layout inside a Function**
def audit_overview_layout():
    note = get_page_note("/audit/annual-census")
    return dbc.Container(
        [
            dbc.Row(
                [
                    dbc.Col(
                        html.H1("Annual Census PDF Survey Audit"),
                        width=12,
                        className="m-1",
                    ),
                ]
            ...
            html.Div(
                id="audit-overview-content",
                style={"display": "none"},
                children=[
                    dbc.Row(
                        [
                            dbc.Col(
                                dcc.Graph(id="audit-region-submission-rate-bar-chart"),
                                md=6,
                                xs=12,
                                className="p-3",
                            ),
                            dbc.Col(
                                dcc.Graph(id="audit-submission-timeliness-bar-chart"),
                                md=6,
                                xs=12,
                                className="p-3",
                            ),
                    ...
                    dbc.Row(
                        [
                            dbc.Col(
                                dcc.Graph(id="audit-submission-rate-map"),
                                md=6,
                                xs=12,
                                className="p-3",
                            ),
                            dbc.Col(
                                dcc.Graph(id="audit-submission-timeliness-map"),
                                md=6,
                                xs=12,
                                className="p-3",
                            ),
...
@dash.callback(
    Output(
        component_id="audit-region-submission-rate-bar-chart",
        component_property="figure",
    ),
    Output(
        component_id="audit-submission-timeliness-bar-chart",
        component_property="figure",
    ),
    ...
    Output(
        "audit-overview-loading-spacer", "style"
    ),  # hide spacer when done; keep while loading/no-data
...
def update_dashboard(selected_year):
    ...
    region_grouped = (
        ...
        .agg(ActiveSchools=("schNo", "count"), SubmittedCount=("Submitted", "sum"))
        .reset_index()
    )
    ...
    # Add a In process label during the year as needed
    ...
    fig_submission_progress.update_traces(
    ...
    df_timing["DaysSinceMarch15"] = df_timing.apply(
        lambda row: (
            row["pCreateDateTime"] - pd.Timestamp(f"{row['svyYear']}-03-15")
        ).days,
        axis=1,
    )
    ...
    timing_grouped = (
        ...
        .agg(AvgDaysAfterMar15="mean", EarliestSubmission="min", LatestSubmission="max")
        .round(1)
        .reset_index()
    )
    ...
    # timing_melted.loc[timing_melted["svyYear"] == 2025, "svyYearLabel"] = (
    #    "2025 (In Process)"
    # )
    ...
    # Prepare data
    df_map["SubmissionStatus"] = df_map["Submitted"].map(
    ...
    df_timely_map["DaysSinceMarch15"] = df_timely_map.apply(
        lambda row: (
            row["pCreateDateTime"] - pd.Timestamp(f"{row['svyYear']}-03-15")
        ).days,
        axis=1,
