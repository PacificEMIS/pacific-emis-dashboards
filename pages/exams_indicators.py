...
dash.register_page(__name__, path="/exams/indicators", name="Exams - Indicators")
...
def create_annotated_mirror_percentage_chart(grouped_df, y_col, group_col, value_col, title, y_label, group_label_func=str, height=600, full_label_col=None, reverse_groups=False, y_order=None):
    ...
    annotations = [dict(x=row_positive_pct.get(f"{label}|{group}", 0) + 2, y=f"{label}|{group}", text=group_label_func(group),
        showarrow=False, font=dict(size=9, color="gray"), xanchor="left") for label in y_values for group in groups]
    tick_vals = [-100, -80, -60, -40, -20, 0, 20, 40, 60, 80, 100]
    tick_text = ["100%", "80%", "60%", "40%", "20%", "0%", "20%", "40%", "60%", "80%", "100%"]
    fig.update_layout(barmode="relative", title=title, height=height,
        xaxis=dict(title="Percentage", tickvals=tick_vals, ticktext=tick_text, range=[-105, 115]),
        yaxis=dict(title=y_label, tickmode="array", tickvals=y_categories, ticktext=y_ticktext, categoryorder="array", categoryarray=y_categories[::-1], tickfont=dict(size=9)),
...
def layout():
    return dbc.Container([
        dbc.Row([dbc.Col(html.H1("Indicators Overview"), width=12, className="m-1")]),
        dbc.Row([
            dbc.Col([html.Label("Year", className="fw-bold"), dcc.Dropdown(id="exams-ind-year-filter", options=year_options, value=default_year, clearable=False)], md=2, className="m-1"),
            dbc.Col([html.Label("Exam", className="fw-bold"), dcc.Dropdown(id="exams-ind-exam-filter", options=[], value=None, placeholder="Select Exam...", clearable=False)], md=4, className="m-1"),
            dbc.Col([html.Label("Analysis", className="fw-bold"), dcc.Dropdown(id="exams-ind-analysis-filter", options=ANALYSIS_TYPE_OPTIONS, value="candidateCount", clearable=False)], md=2, className="m-1"),
        ]),
        dcc.Loading(id="exams-ind-loading", type="default",
            children=html.Div(id="exams-ind-loading-spacer", style={"minHeight": "50vh"},
                children=dbc.Row([dbc.Col(dbc.Alert(id="exams-ind-nodata-msg", color="warning", is_open=False), width=12, className="m-1")]))),
        html.Div(id="exams-ind-content", style={"display": "none"}, children=[
            dbc.Row([dbc.Col(dcc.Graph(id="exams-ind-main-chart"), md=12, className="p-3")], className="m-1"),
            dbc.Row([dbc.Col(dcc.Graph(id="exams-ind-year-chart"), md=12, className="p-3")], className="m-1"),
            dbc.Row([dbc.Col(dcc.Graph(id="exams-ind-district-chart"), md=12, className="p-3")], className="m-1"),
            dbc.Row([dbc.Col(dcc.Graph(id="exams-ind-region-chart"), md=12, className="p-3")], className="m-1"),
            dbc.Row([dbc.Col(dcc.Graph(id="exams-ind-authgovt-chart"), md=12, className="p-3")], className="m-1"),
            dbc.Row([dbc.Col([html.H3(id="exams-ind-table-title"), dash_table.DataTable(id="exams-ind-table", style_table={"overflowX": "auto"},
                style_header={"textAlign": "center", "fontWeight": "bold"}, style_cell={"textAlign": "left", "fontSize": "12px"},
                style_data_conditional=[{"if": {"column_id": "Total"}, "fontWeight": "bold"}])], md=12, className="m-1")]),
        ]),
    ], fluid=True)


@dash.callback(Output("exams-ind-exam-filter", "options"), Output("exams-ind-exam-filter", "value"),
    Input("exams-ind-year-filter", "value"), Input("warehouse-version-store", "data"))
...
@dash.callback(
    Output("exams-ind-main-chart", "figure"), Output("exams-ind-year-chart", "figure"), Output("exams-ind-district-chart", "figure"),
    Output("exams-ind-region-chart", "figure"), Output("exams-ind-authgovt-chart", "figure"), Output("exams-ind-table", "data"),
    Output("exams-ind-table", "columns"), Output("exams-ind-table-title", "children"), Output("exams-ind-nodata-msg", "children"),
    Output("exams-ind-nodata-msg", "is_open"), Output("exams-ind-loading-spacer", "style"), Output("exams-ind-content", "style"),
    Input("exams-ind-year-filter", "value"), Input("exams-ind-exam-filter", "value"), Input("exams-ind-analysis-filter", "value"), Input("warehouse-version-store", "data"))
def update_dashboard(selected_year, selected_exam, analysis_type, _):
    ...
    fig_main = create_mirror_bar_chart_percentage(grouped_main, "DisplayLabel", value_col, f"{exam_name} - Indicators ({analysis_label})", "Indicator", height=main_height, full_label_col="FullLabel")
    if key_order:
    ...
    if not year_data.empty:
        ...
        year_key_order = year_data[["Key", "DisplayLabel"]].drop_duplicates().sort_values("Key")["DisplayLabel"].tolist()
        year_height = max(1000, num_items * len(year_data["examYear"].unique()) * 22)
        fig_year = create_annotated_mirror_percentage_chart(grouped_year, "DisplayLabel", "examYear", value_col, f"Results by Year - {exam_name} ({analysis_label})", "Indicator", lambda y: str(int(y)), height=year_height, full_label_col="FullLabel", reverse_groups=True, y_order=year_key_order)
    else:
        fig_year = {}
    ...
    if not district_data.empty:
        ...
        dist_height = max(1000, num_items * district_data["DistrictName"].nunique() * 35)
        fig_district = create_annotated_mirror_percentage_chart(grouped_dist, "DisplayLabel", "DistrictName", value_col, f"Results by {vocab_district} - {exam_name} ({analysis_label})", "Indicator", str, height=dist_height, full_label_col="FullLabel", y_order=key_order)
    else:
        fig_district = {}
    ...
    if not region_data.empty:
        ...
        reg_height = max(1000, num_items * region_data["RegionName"].nunique() * 20)
        fig_region = create_annotated_mirror_percentage_chart(grouped_reg, "DisplayLabel", "RegionName", value_col, f"Results by {vocab_region} - {exam_name} ({analysis_label})", "Indicator", str, height=reg_height, full_label_col="FullLabel", y_order=key_order)
    else:
        fig_region = {}
    ...
    if not authgovt_data.empty:
        ...
        ag_height = max(800, num_items * authgovt_data["AuthorityGroupName"].nunique() * 20)
        fig_authgovt = create_annotated_mirror_percentage_chart(grouped_ag, "DisplayLabel", "AuthorityGroupName", value_col, f"Results by {vocab_authoritygovt} - {exam_name} ({analysis_label})", "Indicator", str, height=ag_height, full_label_col="FullLabel", y_order=key_order)
    else:
        fig_authgovt = {}
    ...
    if not filtered.empty:
        ...
        pivot = pivot.rename(columns={"DisplayLabel": "Indicator"})
        pivot = pivot[["Indicator"] + level_cols + ["Total"]]
