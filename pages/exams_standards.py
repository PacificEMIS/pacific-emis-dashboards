...
dash.register_page(__name__, path="/exams/standards", name="Exams - Standards")
...
CHART_HEIGHT = 500  # Taller for more items
...
def layout():
    return dbc.Container([
        dbc.Row([dbc.Col(html.H1("Standards Overview"), width=12, className="m-1")]),
        dbc.Row([
            dbc.Col([html.Label("Year", className="fw-bold"), dcc.Dropdown(id="exams-std-year-filter", options=year_options, value=default_year, clearable=False)], md=2, className="m-1"),
            dbc.Col([html.Label("Exam", className="fw-bold"), dcc.Dropdown(id="exams-std-exam-filter", options=[], value=None, placeholder="Select Exam...", clearable=False)], md=4, className="m-1"),
            dbc.Col([html.Label("Analysis", className="fw-bold"), dcc.Dropdown(id="exams-std-analysis-filter", options=ANALYSIS_TYPE_OPTIONS, value="candidateCount", clearable=False)], md=2, className="m-1"),
        ]),
        dcc.Loading(id="exams-std-loading", type="default",
            children=html.Div(id="exams-std-loading-spacer", style={"minHeight": "50vh"},
                children=dbc.Row([dbc.Col(dbc.Alert(id="exams-std-nodata-msg", color="warning", is_open=False), width=12, className="m-1")]))),
        html.Div(id="exams-std-content", style={"display": "none"}, children=[
            dbc.Row([dbc.Col(dcc.Graph(id="exams-std-main-chart"), md=12, className="p-3")], className="m-1"),
            dbc.Row([dbc.Col(dcc.Graph(id="exams-std-year-chart"), md=12, className="p-3")], className="m-1"),
            dbc.Row([dbc.Col(dcc.Graph(id="exams-std-district-chart"), md=12, className="p-3")], className="m-1"),
            dbc.Row([dbc.Col(dcc.Graph(id="exams-std-region-chart"), md=12, className="p-3")], className="m-1"),
            dbc.Row([dbc.Col(dcc.Graph(id="exams-std-authgovt-chart"), md=12, className="p-3")], className="m-1"),
            dbc.Row([dbc.Col([html.H3(id="exams-std-table-title"), dash_table.DataTable(id="exams-std-table", style_table={"overflowX": "auto"},
...
@dash.callback(Output("exams-std-exam-filter", "options"), Output("exams-std-exam-filter", "value"),
    Input("exams-std-year-filter", "value"), Input("warehouse-version-store", "data"))
...
@dash.callback(
    Output("exams-std-main-chart", "figure"), Output("exams-std-year-chart", "figure"), Output("exams-std-district-chart", "figure"),
    Output("exams-std-region-chart", "figure"), Output("exams-std-authgovt-chart", "figure"), Output("exams-std-table", "data"),
    Output("exams-std-table", "columns"), Output("exams-std-table-title", "children"), Output("exams-std-nodata-msg", "children"),
    Output("exams-std-nodata-msg", "is_open"), Output("exams-std-loading-spacer", "style"), Output("exams-std-content", "style"),
    Input("exams-std-year-filter", "value"), Input("exams-std-exam-filter", "value"), Input("exams-std-analysis-filter", "value"), Input("warehouse-version-store", "data"))
def update_dashboard(selected_year, selected_exam, analysis_type, _):
    ...
    fig_main = create_mirror_bar_chart_percentage(grouped_main, "DisplayLabel", value_col, f"{exam_name} - Standards ({analysis_label})", "Standard", height=main_height, full_label_col="FullLabel")
    if key_order:
    ...
    if not year_data.empty:
        ...
        year_height = max(600, num_items * len(year_data["examYear"].unique()) * 35)
        fig_year = create_annotated_mirror_percentage_chart(grouped_year, "DisplayLabel", "examYear", value_col, f"Results by Year - {exam_name} ({analysis_label})", "Standard", lambda y: str(int(y)), height=year_height, full_label_col="FullLabel", reverse_groups=True, y_order=key_order)
    else:
        fig_year = {}
    ...
    if not district_data.empty:
        ...
        fig_district = create_annotated_mirror_percentage_chart(grouped_dist, "DisplayLabel", "DistrictName", value_col, f"Results by {vocab_district} - {exam_name} ({analysis_label})", "Standard", str, height=dist_height, full_label_col="FullLabel", y_order=key_order)
    else:
        fig_district = {}
    ...
    if not region_data.empty:
        ...
        reg_height = max(600, num_items * region_data["RegionName"].nunique() * 30)
        fig_region = create_annotated_mirror_percentage_chart(grouped_reg, "DisplayLabel", "RegionName", value_col, f"Results by {vocab_region} - {exam_name} ({analysis_label})", "Standard", str, height=reg_height, full_label_col="FullLabel", y_order=key_order)
    else:
        fig_region = {}
    ...
    if not authgovt_data.empty:
        ...
        ag_height = max(500, num_items * authgovt_data["AuthorityGroupName"].nunique() * 30)
        fig_authgovt = create_annotated_mirror_percentage_chart(grouped_ag, "DisplayLabel", "AuthorityGroupName", value_col, f"Results by {vocab_authoritygovt} - {exam_name} ({analysis_label})", "Standard", str, height=ag_height, full_label_col="FullLabel", y_order=key_order)
    else:
        fig_authgovt = {}
    ...
    if not filtered.empty:
        ...
        pivot = pivot.rename(columns={"DisplayLabel": "Standard"})
        pivot = pivot[["Standard"] + level_cols + ["Total"]]
